...
exports.login = async (req, res) => {
  try {
    ...
    const isPasswordMatch = await auth.checkPassword(req.body.user.password, user);
    ...
    if (!isPasswordMatch) {
      ...
      await saveLoginAttemptsToDB(user)
      utils.handleSuccess(res, utils.buildSuccObject({
...
exports.requireAuth =  async (req, res, next) => {
  passport.authenticate('jwt', (_, user, err) => {
    if (err) return utils.handleError(res, utils.buildErrObject(422, err.message));
    if (!user) return utils.handleError(res, utils.buildErrObject(422, 'Unknown user'));
