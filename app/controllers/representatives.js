...
exports.updateRepresentative = async (req, res) => {
  Category.updateOne({ slug: req.params.slug, 'representatives._id': req.params.representativeId }, {
    $set: {
  ...
  .then(result => {
    if (result.n) {
      if (result.nModified) handleSuccess(res, buildSuccObject('Representative updated'));
...
exports.deleteRepresentative = async (req, res) => {
  ...
  .then(result => {
    if (result.n) {
      if (result.nModified) handleSuccess(res, buildSuccObject('Representative removed'));
