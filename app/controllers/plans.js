...
exports.updatePlan = async (req, res) => {
  Category.updateOne({ slug: req.params.slug, 'plans._id': req.params.planId }, {
    $set: {
  ...
  .then(result => {
    if (result.n) {
      if (result.nModified) handleSuccess(res, buildSuccObject('Plan updated'));
...
exports.deletePlan = async (req, res) => {
  ...
  .then(result => {
    if (result.n) {
      if (result.nModified) handleSuccess(res, buildSuccObject('Plan removed'));
